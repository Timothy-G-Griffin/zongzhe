...
Lemma bop_add_id_congruence : bop_congruence (cas_constant + S ) (c <+> rS) (c [+] bS). 
Proof. unfold bop_congruence. intros [s1 | t1] [s2 | t2] [s3 | t3] [s4 | t4]; simpl; intros H1 H2;auto; discriminate. Qed. 

Lemma bop_add_id_associative : bop_associative S rS bS -> bop_associative (cas_constant + S ) (c <+> rS) (c [+] bS). 
Proof. intros assS [s1 | t1] [s2 | t2] [s3 | t3]; simpl; auto. Qed. 

Lemma bop_add_id_idempotent : bop_idempotent S rS bS → bop_idempotent (cas_constant + S ) (c <+> rS) (c [+] bS). 
Proof. intros idemS [s1 | t1]; simpl; auto. Qed. 

Lemma bop_add_id_not_idempotent : bop_not_idempotent S rS bS → bop_not_idempotent (cas_constant + S ) (c <+> rS) (c [+] bS). 
Proof. intros [s P]. exists (inr _ s). simpl. assumption. Defined. 

Lemma bop_add_id_commutative : bop_commutative S rS bS → bop_commutative (cas_constant + S ) (c <+> rS) (c [+] bS). 
Proof. intros commS [s1 | t1] [s2 | t2]; simpl; auto. Qed. 

Lemma bop_add_id_not_commutative : bop_not_commutative S rS bS → bop_not_commutative (cas_constant + S ) (c <+> rS) (c [+] bS). 
Proof. intros [ [s t] P]. exists (inr _ s, inr _ t); simpl. assumption. Defined. 

Lemma bop_add_id_selective : bop_selective S rS bS → bop_selective (cas_constant + S ) (c <+> rS) (c [+] bS). 
Proof. intros selS [s1 | t1] [s2 | t2]; simpl; auto. Qed. 

Lemma bop_add_id_not_selective : bop_not_selective S rS bS → bop_not_selective (cas_constant + S ) (c <+> rS) (c [+] bS). 
Proof. intros [ [s1 s2] P]. exists (inr _ s1, inr _ s2). simpl. assumption. Defined. 


Lemma bop_add_id_exists_id : bop_exists_id (cas_constant + S ) (c <+> rS) (c [+] bS).
Proof. exists (inl S c). intros [a | b]; compute; auto. Defined. 

Lemma bop_add_id_exists_ann : bop_exists_ann S rS bS -> bop_exists_ann (cas_constant + S ) (c <+> rS) (c [+] bS).
Proof. intros [annS pS]. exists (inr _ annS). intros [s | t]; compute; auto. Defined. 

Lemma bop_add_id_not_exists_ann (s : S) : bop_not_exists_ann S rS bS -> bop_not_exists_ann (cas_constant + S ) (c <+> rS) (c [+] bS).
